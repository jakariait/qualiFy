...
export default function ExamForm({ initialData = {}, onSuccess }) {
  ...
  const fetchExamBasicInfo = async () => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/exams/${initialData._id}?subjectsPage=1&subjectsLimit=3&questionsPage=0`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      if (data.pagination?.subjects) {
        ...
        const counts = {};
        data.subjects?.forEach((sub, idx) => { counts[idx] = sub.questionsPagination?.total || 0; });
  ...
  useEffect(() => { fetchExamBasicInfo(); }, [initialData?._id]);
  ...
  const loadMoreSubjects = async () => {
    ...
    try {
      ...
      setForm(prev => ({ ...prev, subjects: [...prev.subjects, ...res.data.subjects] }));
